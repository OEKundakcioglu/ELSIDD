...
# Y -> z
        ...
        for i, t in dataset["c"].keys():
            mdl.add_constraint(I[i, t - 1] + x[i, t] == I[i, t]+ (mdl.sum(Y[i, t,s+1] for s in range(store_number))) , "cons1_i={},t={}".format(i, t))
            mdl.add_constraint(I_prime[i, t - 1] + (mdl.sum(Y[i, t,s+1] for s in range(store_number)))== D[i, t]+I_prime[i, t] , "cons2_i={},t={}".format(i, t))
            mdl.add_constraint(U[i, t] == I[i, t - 1] +(mdl.sum(Y[i, t,s+1] for s in range(store_number))), "cons3_i={},t={}".format(i, t))
        ...
        for s in range(len(C_store)):
            mdl.add_constraint(mdl.sum(Y[i, t,s+1] for i in range(1, N + 1)  for t in range(1, T + 1)) <= C_store[s],"cons14_t={}".format(t))
        ...
        trans_cost=mdl.sum(dataset["h"][i, t] * Y[i, t,s] for i, t,s in u_prime.keys()) # h f olacak unit cost trans
        trans_setup_cost=mdl.sum(((dataset["C"][t])/5)* z[i,t,s] for i,t,s in u_prime.keys())  #C F olacak  fixed cost trans
